...
with st.sidebar:
    ...
    model_choice = st.selectbox(
        "Choose LLM Interface", ["Local (Ollama)", "Cloud (OpenRouter)"]
    )
    ...
    if model_choice == "Local (Ollama)":
        local_model_name = st.text_input(
            ...
            value="mistral",
            help="Type the name of any model downloaded via Ollama (e.g., 'llama3.2', 'mistral', 'gemma:2b', 'qwen2.5')"
        )

    # Initialize connection to Language Model via the engine wrapper
    llm = instantiate_llm(model_choice, local_model_name=local_model_name)

    if llm and model_choice == "Cloud (OpenRouter)":
        st.success("API Key loaded from secrets successfully!")
    elif not llm and model_choice == "Cloud (OpenRouter)":
        st.error("No API key found in secret.txt or Streamlit secrets.")

    st.markdown("---")
    top_k = st.slider("Top-k Document Retrieval", min_value=1, max_value=8,
                      value=3, help="How many knowledge chunks to inject into the LLM logic.")
